...
<div class="main-container">
  ...
  <div class="content-container">
      <?php
        ...
        $userID = $_POST['adminUser'];
    
        $query = "SELECT * FROM User WHERE userID = $userID";
        $result = mysqli_query($db, $query);
    
        if(mysqli_num_rows($result) > 0){
            $query = "UPDATE User SET user_type='ADMIN' WHERE userID = $userID";
            $result = mysqli_query($db, $query);
            if($result){
                print"<h2>You successfully gave admin to a user.</h2>";
            }
            else{
                print"<h2>Was unable to get user.  Try again!</h2>";
            }
        }
        else{
            print"<h2>No user exists.</h2>";
        }
      ?>
